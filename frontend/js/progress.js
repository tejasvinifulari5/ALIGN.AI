...
async function initProgress() {
    ...
    try {
        const res = await fetch(`http://127.0.0.1:5500/progress/${user.id}`);
        const result = await res.json();
        
        if (!result.success) return;

        // Map DB fields to the expected JS structure
        const all = (result.sessions || []).map(s => {
            const dateObj = new Date(s.session_date);
            const dateShort = isNaN(dateObj) ? s.session_date : dateObj.toLocaleDateString("en-IN", {day:"numeric", month:"short"});
            
            let rating = "Keep Practicing 🔄";
            if (s.posture_score >= 90) rating = "Excellent 🏆";
            else if (s.posture_score >= 75) rating = "Very Good 💪";
            else if (s.posture_score >= 60) rating = "Good 👍";

            return {
                exercise: s.exercise_name.replace(/_/g, " "),
        ...
        // Make filterScore globally accessible via window since it's used in HTML onclick
        window.filterScore = function(ex, btn) {
